/**
 * @file
 * @brief ConstantUtil_FILE_PATHヘッダーファイル
 */
#pragma once


#include "ConstantUtil.h"


namespace cpp_base {
namespace ConstantUtil {
namespace FILE_PATH {
	const WCHAR SYSTEM_CONFIG[] = L"dat/sys_conf.ini";
	const WCHAR BACKGROUND_IMAGE[] = L"res/bg_img.png";
	const WCHAR EMPTY_BACKGROUND_IMAGE[] = L"res/empty_bg_img.png";
	const WCHAR TITLE_BACKGROUND_IMAGE[] = L"res/title_bg_img.png";
	const WCHAR TITLE_LOGO_IMAGE[] = L"res/title_logo_img.png";
	const WCHAR PLAYER_2D_IMAGE[] = L"res/pl_2d_img.png";
	const WCHAR MOB_2D_IMAGE[] = L"res/mob_2d_img.png";
	const WCHAR SHADOW_2D_IMAGE[] = L"res/shadow_2d_img.png";
	const WCHAR COMMON_ATLAS[] = L"res/common_atlas.plist";
	const WCHAR GROUND_2D_MAP[] = L"res/ground_2d_map.tmx";
	const WCHAR TITLE_BGM_SOUND[] = L"res/title_bgm_sound.mp3";
	const WCHAR SELECT_BGM_SOUND[] = L"res/select_bgm_sound.mp3";
	const WCHAR START_SE_SOUND[] = L"res/start_se_sound.mp3";
	const WCHAR INIT_SCENE_PREFAB[] = L"res/init_scene_prefab.xml";
	const WCHAR TITLE_SCENE_PREFAB[] = L"res/title_scene_prefab.xml";
	const WCHAR SELECT_SCENE_PREFAB[] = L"res/select_scene_prefab.xml";
	const WCHAR STAGE_SCENE_PREFAB[] = L"res/stage_scene_prefab.xml";
	const WCHAR TEST_2D_STAGE_NODE_PREFAB[] = L"res/test_2d_stage_node_prefab.xml";
	const WCHAR FIELD_2D_NODE_PREFAB[] = L"res/field_2d_node_prefab.xml";
}
}
}
