 ...
 * 0未満=失敗
 */
INT tml::sound::SoundDesc::ReadValue(const tml::INIFile &conf_file)
{
	if (tml::sound::ManagerResourceDesc::ReadValue(conf_file) < 0) {
		return (-1);
	}
...
INT tml::sound::Sound::OnCreateDeferred(void)
{
	...
	sound_file.read_desc.parent_data = sound_file_read_desc_dat;

	if (sound_file.Read() < 0) {
		return (-1);
	}

	auto &sound_file_buf = sound_file.data.buffer;

	if (sound_file_buf.GetLength() <= 0U) {
		return (-1);
	}
	...
	do {
		// WAVE File Check
		if ((sound_file_buf.GetLength() >= 4U)
		&& (strncmp(reinterpret_cast<CHAR *>(sound_file_buf.Get()), "RIFF", 4U) == 0)) {
			file_format = 1U;
		...
		// MP3 File Check
		if ((sound_file_buf.GetLength() >= 3U)
		&& (strncmp(reinterpret_cast<CHAR *>(sound_file_buf.Get()), "ID3", 3U) == 0)) {
			file_format = 2U;
		...
		// Ogg File Check
		if ((sound_file_buf.GetLength() >= 4U)
		&& (strncmp(reinterpret_cast<CHAR *>(sound_file_buf.Get()), "OggS", 4U) == 0)) {
			file_format = 3U;
	...
	case 1: {// WAVE File Read
		...
		mmio_info.cchBuffer = sound_file_buf.GetLength();
		mmio_info.pchBuffer = reinterpret_cast<HPSTR>(sound_file_buf.Get());
	...
	case 2: {// MP3 File Read
		if ((sound_file_buf.GetLength() >= 128U)
		&& (strncmp(reinterpret_cast<CHAR *>(&sound_file_buf.Get()[sound_file_buf.GetLength() - 128U]), "TAG", 3U) == 0)) {
			sound_file_buf.SetLength(sound_file_buf.GetLength() - 128U);
		}

		size_t sound_file_buf_index = 0U;

		std::array<BYTE, 10U> mp3_tag_header;

		if (sound_file_buf.GetLength() < (sound_file_buf_index + mp3_tag_header.size())) {
			return (-1);
		}

		tml::Copy(mp3_tag_header.data(), &sound_file_buf.Get()[sound_file_buf_index], mp3_tag_header.size());

		size_t mp3_tag_size = ((mp3_tag_header[6] << 21) | (mp3_tag_header[7] << 14) | (mp3_tag_header[8] << 7) | mp3_tag_header[9]) + mp3_tag_header.size();

		if (sound_file_buf.GetLength() < (sound_file_buf_index + mp3_tag_size)) {
			return (-1);
		}

		sound_file_buf_index += mp3_tag_size;

		std::array<BYTE, 4U> mp3_frame_header;

		if (sound_file_buf.GetLength() < (sound_file_buf_index + mp3_frame_header.size())) {
			return (-1);
		}

		tml::Copy(mp3_frame_header.data(), &sound_file_buf.Get()[sound_file_buf_index], mp3_frame_header.size());
		...
		DWORD mp3_buf_size = sound_file_buf.GetLength() - sound_file_buf_index;
		...
		acm_stream_header.pbSrc = &sound_file_buf.Get()[sound_file_buf_index];
	...
	case 3: {// Ogg File Read
		this->ogg_file_buf_ = sound_file_buf.Get();
		this->ogg_file_buf_size_ = sound_file_buf.GetLength();
		this->ogg_file_buf_index_ = 0L;
